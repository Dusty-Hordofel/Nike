...
const DeliveryModeSelector: React.FC<DeliveryModeSelectorProps> = ({
  ...
  return (
    // mb-5
    <div role="radiogroup" className="flex gap-x-4 px-5 bg-green-500">
      <div
        role="radio"
        aria-checked={selectedMode === DeliveryMode.Shipping}
        className={`w-full px-5 py-4  rounded-lg ${selectedMode === DeliveryMode.Shipping ? "border-black-200 border-2" : "border-gray-500 border"} cursor-pointer`}
        tabIndex={0}
      ...
      >
        <h3 className="flex items-center justify-center h-full">
          ...
          <span className="h-full flex items-center ">Expédition</span>
        </h3>
      </div>
      <div
        role="radio"
        aria-checked={selectedMode === DeliveryMode.Pickup}
        className={`w-full px-5 py-3 border rounded-lg ${selectedMode === DeliveryMode.Pickup ? "border-black-200 border-2" : "border-gray-500 border"} cursor-pointer`}
        tabIndex={0}
      ...
      >
        <h3 className="flex items-center justify-center h-full">
          ...
          <span className="h-full flex items-center ">Retrait</span>
        </h3>
      </div>
    </div>
  );
